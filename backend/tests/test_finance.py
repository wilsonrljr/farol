...
class TestFinanceCalculations(unittest.TestCase):
    ...
    def test_simulate_with_amortizations(self):
        ...
        price_result = simulate_price_loan(
            loan_value, term_months, monthly_interest_rate, amortizations
        )
    ...
    def test_recurring_amortizations_percentage_and_fixed(self):
        ...
        extras = [
            i.extra_amortization
            for i in sac_result.installments
            if i.month % 6 == 0 and i.month <= 36
        ]
    ...
    def test_amortization_validator_conflict(self):
        ...
        with self.assertRaises(ValueError):
            AmortizationInput(
                month=6, value=1000, interval_months=12, occurrences=3, end_month=60
            )
    ...
    def test_inflation_adjusted_recurring_amortization(self):
        ...
        extras = {
            inst.month: inst.extra_amortization
            for inst in result.installments
            if inst.extra_amortization > 0
        }
    ...
    def test_months_saved_and_total_extra_amortization_fields(self):
        ...
        monthly_interest_rate = 0.8
        amortizations = [
            AmortizationInput(month=12, value=20000),
            AmortizationInput(month=24, value=15000),
        ]
